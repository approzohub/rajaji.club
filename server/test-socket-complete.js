...
async function testSocketComplete() {
  try {
    console.log('🧪 Testing complete socket event flow...\n');
    ...
    // Don't delete the test user as it might be used again
    console.log('✅ Test user kept for future tests');

    console.log('\n🎉 Test completed! Check server logs for socket emission messages.');

  } catch (error) {
    console.error('❌ Error testing socket complete flow:', error);
  } finally {
    mongoose.disconnect();
  }
}
